...
prompt_pure_preprompt_render() {
	...
	# assume dir names are 8 chars in average + 1 path separator = 9
	let max_elems=$left_len/9

	# make sure to list at list one dir
	[[ $max_elems -gt 0 ]] || max_elems=1
	prompt_exp="%($((max_elems + 1))~|.../%${max_elems}~|%~)"

	#preprompt_parts+=("%B%F{blue}%$len<...<%3~%<<%f%b")
	preprompt_parts+=("%B%F{blue}$~prompt_exp%f%b")
...
prompt_pure_async_vcs_info() {
	...
	zstyle ':vcs_info:hg*' formats '%b' '%R' '%s'
	zstyle ':vcs_info:hg*' actionformats '%b|%a' '%R' '%s'
	...
	info[system]=${vcs_info_msg_2_%-svn}

	if [[ -z $info[branch] ]]; then
		for v in $PUREPROMPT_VCS_CMDS; do
			eval ${v} 2>/dev/null
		done
	fi
...
prompt_pure_async_git_dirty() {
	...
	# abort if not git
	[[ $prompt_pure_vcs_info[system] != 'git' ]] && return
...
prompt_pure_async_callback() {
	...
	case $job in
		prompt_pure_async_vcs_info)
			...
			prompt_pure_vcs_info[system]=$info[system]
...
prompt_pure_setup() {
	...
	typeset -gU PUREPROMPT_VCS_CMDS
	typeset -gU PUREPROMPT_TITLE_CMDS
	...
	# show username@host if logged in through SSH and not TMUX (already shows host)
